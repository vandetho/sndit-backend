...
#[Tag(name: 'Internal Ticket')]
#[Route(path: '/tickets', name: 'sndit_internal_api_tickets_')]
class TicketController extends AbstractController
{
    ...
    public function gets(Request $request): JsonResponse
    {
        ...
        return $this->generateTableData(
            $this->ticketRepository->findByCriteria(
                $criteria,
                $this->getOrderBy($request),
                ...$this->getOffsetAndLimit($request)
            ),
            $this->ticketRepository->countByCriteria($criteria),
            'tickets'
        );
    }
    ...
    #[OA\Response(
        response: Response::HTTP_OK,
        description: 'Return when ticket need feedbacks',
        content: new OA\JsonContent(properties: [
            new OA\Property(property: 'error', type: 'bool', enum: [false]),
            new OA\Property(property: 'message', type: 'string'),
            new OA\Property(property: 'data', ref: new Model(type: Ticket::class)),
    ...
    #[Route(path: '/{idOrToken}/need-feedbacks', name: 'need_feedbacks', methods: ['POST'])]
    public function needFeedbacks(int|string $idOrToken, Request $request): JsonResponse
    {
        /** @var CheckTicketExistEvent $event */
        $event = $this->dispatchEvent(CheckTicketExistEvent::class, $idOrToken);
        if ($event->getResponse()) {
        ...
        return $this->ticketWorkflow->onNeedFeedback($event->getTicket(), $request);
    }
    ...
    public function messages(int|string $idOrToken, Request $request): JsonResponse
    {
        /** @var CheckTicketExistEvent $event */
        $event = $this->dispatchEvent(CheckTicketExistEvent::class, $idOrToken);
        if ($event->getResponse()) {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            $ticketMessage->setInternalUser($this->getUser());
    ...
    #[OA\Response(
        response: Response::HTTP_OK,
        description: 'Return when ticket is rejected',
        content: new OA\JsonContent(properties: [
            new OA\Property(property: 'error', type: 'bool', enum: [false]),
            new OA\Property(property: 'message', type: 'string'),
            new OA\Property(property: 'data', ref: new Model(type: Ticket::class)),
    ...
    #[Route(path: '/{idOrToken}/rejects', name: 'rejects', methods: ['POST'])]
    public function rejects(int|string $idOrToken): JsonResponse
    {
        /** @var CheckTicketExistEvent $event */
        $event = $this->dispatchEvent(CheckTicketExistEvent::class, $idOrToken);
        if ($event->getResponse()) {
        ...
        return $this->ticketWorkflow->onReject($event->getTicket());
    }
    ...
    #[OA\Response(
        response: Response::HTTP_OK,
        description: 'Return when ticket is treated',
        content: new OA\JsonContent(properties: [
            new OA\Property(property: 'error', type: 'bool', enum: [false]),
            new OA\Property(property: 'message', type: 'string'),
            new OA\Property(property: 'data', ref: new Model(type: Ticket::class)),
    ...
    #[Route(path: '/{idOrToken}/treats', name: 'treats', methods: ['POST'])]
    public function treats(int|string $idOrToken): JsonResponse
    {
        /** @var CheckTicketExistEvent $event */
        $event = $this->dispatchEvent(CheckTicketExistEvent::class, $idOrToken);
        if ($event->getResponse()) {
        ...
        return $this->ticketWorkflow->onTreat($event->getTicket());
    }
}
